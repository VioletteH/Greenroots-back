...
export const getAll = async (limit=10, offset=0): Promise<{ trees: Tree[]; total: number }> => {
    const response = await axios.get(`${api_url}/with-count?limit=${limit}&offset=${offset}`);
    const data = response.data;
    return data;
};

export const getAllWithoutCount = async (limit=10, offset=0): Promise<{ trees: Tree[]; total: number }> => {
