...
//     user: catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
//         next();
//     }),
//     admin: catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
//         const token = req.headers.authorization?.split(' ')[1];
//         if (!token) {
//             res.status(401).json({ message: 'Unauthorized' });
//             return;
